...
# ---------------------------
# Indicator extraction (legacy metrics you already had)
# ---------------------------
RX = {
...
def extract_indicators(s: str):
    ...
    found["SourceName"] = set(m.group(0).lower() for m in RX["SourceName"].finditer(s))
    found["ProviderGuid"] = set(m.group(0).lower() for m in RX["ProviderGuid"].finditer(s))
    return found
...
def indicator_scores(gen: str, ref: str):
    ...
    keys = ["eventid","provider_full","channel","username","proc_image","path_any","ipv4","tech_id","SourceName","ProviderGuid"]
    per = {}
    for k in keys:
...
def extract_indicator_items(s: str):
    """
    Flatten all indicator matches into one set of (type, value) pairs.
    This lets us compute a single micro-averaged P/R/F1 across the whole log.
    """
...
def indicator_micro_scores(gen: str, ref: str):
    """
    Micro-averaged precision/recall/F1 across ALL indicator types pooled together.
    """
...
# ---------------------------
# Count + truncate top-level JSON objects
# ---------------------------
...
def truncate_after_n_json(text: str, n: int = 1) -> str:
...
# ---------------------------
# Regeneration of ONE log (progressively stricter settings)
# ---------------------------
def regenerate_one_log(model, tokenizer, prompt, max_new_tokens):
    """
    Retry generating ONE log with progressively stronger settings.
    Returns best text observed (may still be 0 as a last resort).
    """
    tail_variants = [
        # pass 1 (deterministic, concise)
        "Output exactly one Windows event log in JSON. No explanations. One object only.",
        # pass 2 (allow light sampling)
        "Output exactly one Windows event log in JSON. No explanations. Avoid long GUID/hash sequences. Keep each value concise.",
        # pass 3 (even tighter)
        "Output exactly one Windows event log in JSON. No explanations. Do not include documentation or multi-paragraph text. Limit strings to short values.",
    ]
    ...
    for tail in tail_variants:
        ...
        for cfg in decode_cfgs:
            ...
            txt = truncate_after_n_json(extract_response(dec), n=1)
        ...
        # final try with a slightly larger budget before switching instruction
        with torch.no_grad():
            out = model.generate(
                **inputs,
...
# ---------------------------
# Base-Log fields macro P/R/F1 (NEW)
# ---------------------------
    ...
    def walk(d):
        ...
        for k, v in d.items():
            if isinstance(v, dict):
                ...
                if not m:
                    continue
                fld = m.group("key")  # strip any '|operator'
                req.append((fld, v))
    walk(fields)
    return req
...
def _get_value_by_path(obj: dict, field: str):
    """Supports dotted paths; applies key aliases per hop."""
...
def fields_prf_on_prompt_and_gen(prompt_text: str, generated_text: str):
    ...
    required = _collect_required_fields(tmpl)  # list[(field, value)]
    R = len(required)

    # Try to parse the first JSON line from generated_text
    line = generated_text.strip().splitlines()[0].strip() if generated_text.strip() else ""
    parsed = True
    try:
        gen = json.loads(line)
    ...
    per = []
    tp = 0
    fp = 0   # treat "attempted but wrong value" as FP
    fn = 0
    ...
    if parsed:
        for fld, want in required:
            ...
            if got_s is None:
                ...
                ok = want_s in got_s  # relaxed substring match
                if ok:
                    tp += 1
                else:
                    fp += 1
        ...
        # fallback: raw-text presence check
...
def main(args):
    ...
    for ex in tqdm(samples):
        ...
        # Hard cut after the 1st top-level JSON (prevents trailing chatter)
        gen_text = truncate_after_n_json(gen_text, n=1)

        # If we got <1 log, regenerate the full response for ONE log
        if count_top_level_json(gen_text) < 1:
            gen_text2 = regenerate_one_log(
                model, tokenizer, prompt,
                max_new_tokens=min(512, max(192, args.max_new_tokens))  # small budget bump allowed
            )
        ...
        exact_one_flags.append(int(cnt == 1))
        ...
        if has_reference:
            ...
            # --- Indicator metrics (legacy macro) ---
    ...
    if has_reference:
        ...
        print("\n------ Indicator Macro Scores (content overlap vs. reference) ------")
    ...
    print(f"% samples with exactly 1 log: {100.0 * sum(exact_one_flags)/max(1,len(exact_one_flags)):.1f}%")
...
if __name__ == "__main__":
    ...
    ap.add_argument("--max_new_tokens", type=int, default=384)  # 1 log needs less budget
    args = ap.parse_args()
    main(args)
